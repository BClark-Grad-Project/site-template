module.exports = function (data) {
	...
	// EDIT CONTENT VIEWS
	// Main View
	router.get('/content', data.auth.grantAdmin, function(req, res, next){
		res.render('admin/content/content', {title:"coming soon", user: req.session.user });
	});
	
	// EDIT USER PROFILE
	// Main view
	router.get('/users', data.auth.grantAdmin, function(req, res, next){
		res.render('admin/user/users', {title:"coming soon", user: req.session.user });
	});
	
	// Search view
	router.post('/users/search', data.auth.grantAdmin, function(req, res, next){
	...
	// Edit View
	router.get('/users/edit/:profile', data.auth.grantAdmin, function(req, res, next){		
	...
	}).post('/users/edit', data.auth.grantAdmin, function(req, res, next){
	...
	// Create View
	router.post('/users/create', data.auth.grantAdmin, function(req, res, next){
